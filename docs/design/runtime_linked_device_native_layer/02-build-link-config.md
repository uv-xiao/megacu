...
platform/backend/scheduler capability. It consumes task submissions and
component-provided attributes supplied by each `OrchTarget` at runtime. It must
not be named or implemented as a GEMM+AllReduce-only dispatcher.
...
megacu_add_orchestrate_target(
  TARGET gemm_allreduce_phased
  COMPONENTS cuda_nvshmem_static
  SOURCES gemm_allreduce_phased_orchestrate.cc
  PROGRESS phased)
```

The CMake target records linked runtime components and a progress model, not a
materialized program or separate argument schema:
...
dispatcher: annotated_runtime
scheduler: explicit_asap
target signature: target-specific pointers, POD descriptors, and scalars
operator symbols: linked native CUDA/NVSHMEM functions
driver resources: CUDA stream/device resource, NVSHMEM team/PE resource
supported deployment: 1-host/1-device and 1-host/2-device for PR #4
...
- orchestrate entry and target argument signature;
- submitted task/operator structure;
...
- the general dispatcher algorithm;
...
- link the selected components and operator symbols into one
  target;
- expose an orchestrate entry that receives a platform/backend driver plus
...
- dispatcher retargeting between 1-host/1-device and 1-host/2-device work based
  on backend resources exposed through the driver;
- dispatcher tile iteration based on `problem.m`, `problem.n`, `tile_m`, and
  `tile_n`;
- dispatcher peer identity based on the backend resource;
- scheduler progress choices inside the linked ASAP scheduler envelope;
...
orchestrate target
  ...
  links thin platform/backend fact and primitive helpers
```
...
- missing or invalid required target arguments;
