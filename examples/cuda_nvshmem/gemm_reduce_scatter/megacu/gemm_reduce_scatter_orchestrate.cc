#include "examples/cuda_nvshmem/gemm_reduce_scatter/megacu/gemm_reduce_scatter_arena.cuh"
...
void copy_host_arena(gemm_rs::host_arena_storage const &host,
...
megacu::status require_stream(gemm_rs_driver driver) {
...
megacu::status cuda_nvshmem_gemm_reduce_scatter_host_orch(
  ...
  auto args = gemm_rs::runtime_args{.driver = driver,
                                    ...
                                    .events = events,
                                    .problem = problem};
  gemm_rs::host_arena_storage host_arena;
  status = gemm_rs::build_host_arena(host_arena, args);
  if (status.code != megacu::status_code::ok) {
    return status;
  }
  ...
  status = megacu_cuda_gemm_reduce_scatter_host_orch_f32(
...
megacu::status cuda_nvshmem_gemm_reduce_scatter_seeded_orch(
  ...
  managed_arena arena;
  status = arena.allocate(gemm_rs::kTaskCapacity, gemm_rs::kEventCapacity,
                          gemm_rs::kDepCapacity, gemm_rs::kRegionCapacity);
  if (status.code != megacu::status_code::ok) {
    arena.release();
    return status;
  }
  ...
  *construction_status = 0;
  ...
  status = megacu_cuda_gemm_reduce_scatter_seeded_orch_f32(
