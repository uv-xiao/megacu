...
This example will demonstrate all-gather producer work making peer-visible
input regions available to GEMM tile consumer work through the CUDA+NVSHMEM
backend.
...
```bash
cmake -S . -B build
cmake --build build --target cuda_nvshmem_allgather_gemm_megacu
ctest --test-dir build -R '^ag_gemm_example_layout$' --output-on-failure
```
...
under `examples/cuda_nvshmem/allgather_gemm/megacu/` and provide an AG-GEMM
correctness entrypoint for local and distributed launch modes.
...
- Direct, MPI, and Torch launch adapters are required by the current PR and are
  ...
  not yet add a numeric AG-GEMM distributed run.
